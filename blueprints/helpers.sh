...
tf-apply () {
  ...
  PLANFILE="$SCRIPTDIR/$ROOT/tfplan.txt"
  rm "$TF_LOG_PATH" 2> /dev/null || INFO "No previous log found."
  retry 3 "terraform -chdir=$SCRIPTDIR/$root apply -target=module.vpc -auto-approve $PLANFILE"
  INFO "Apply target module.vpc completed."
  retry 3 "terraform -chdir=$SCRIPTDIR/$root apply -target=module.eks -auto-approve $PLANFILE"
  INFO "Apply target module.eks completed."
  retry 3 "terraform -chdir=$SCRIPTDIR/$root apply -auto-approve $PLANFILE"
